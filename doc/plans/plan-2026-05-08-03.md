...
- Solution / implementation approach: `--apply` flag (after positional
  ...
  - For each `SURGICAL_PATHS` entry: do **not** copy. The same
    reporting line as check mode is emitted — `path: differs
    (surgical merge required)` if the file exists with different
    content, or `path: missing downstream (surgical — not
    auto-copied)` if absent — and the maintainer merges by hand.
...
- T3 keeps `--apply`'s safety story to one rule (refuse dirty
  ...
  uses `install -m` with the source's mode (preferring GNU
  `stat -c '%a'` for Linux/CI, falling back to BSD `stat -f '%Lp'` on
  macOS) so githook executable bits survive.
- T4's "refuse template-rust to itself" case fires when the script's
  own working tree is passed as a downstream argument. The
  "must run from template-rust" guard is more involved: a git-toplevel
  check alone is insufficient (a stale copy in a downstream repo would
  pass), so it's paired with a `.template-rust-root` marker file at
  the template root. The marker is intentionally absent from
  VERBATIM_PATHS / SURGICAL_PATHS so downstream repos never receive
  it, and the guard refuses with exit 2 if the marker is missing.
- T5 covers ten cases (the nine the plan listed — pristine match,
  edited drift, missing path, --apply, dirty refusal, non-git
  downstream, duplicate arg, self-sync, no-args usage — plus an
  exec-bit preservation test that drifts a hook file under --apply
  and asserts the install -m mode round-trips). The test parses the
  manifest from the script, so adding a path is a one-line script
  edit and the test still passes without touching fixtures.
